...
import static com.killer.Light.Constant.*;
/**
...
public class Ball {
    ...
    int muRHandle;//求半径属性引用
    int maPositionHandle;//定点位置属性引用
    ...
    public void initVertexData()
    {
        ...
        final int angleSpan = 10;//将球进行单位切分角度
        ...
        vCount = alVertix.size() / 3;//顶点个数为坐标值的1/3

        //将alVertix中的坐标值转化成一个float数组
        float vertices[] = new float[vCount * 3];
        for (int i = 0; i < alVertix.size(); i++) {
            vertices[i] = alVertix.get(i);
        }
        //创建定点缓冲
        //vertices.length * 4因为一个整数四个字节
        ...
        mNormalBuffer = nbb.asFloatBuffer();//转换成float缓冲
        //向缓冲区注入数据
